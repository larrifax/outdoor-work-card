...
test("carwash defaults: 0.2 mm wets roads, 3 h to dry, no parked window; night_max is ignored", () => {
  const r = wash();
  expect(r.wash.okRain).toBe(0.2);
  expect(r.wash.dryRoadsHours).toBe(3);
  expect(r.wash.parked).toBeNull();
...
test("work tasks: defaults are Mow ≤ 0.3 mm and Paint ≤ 0.1 mm + 24 h after; old `before` falls back to 0.3", () => {
  const def = resolve({ type: "custom:outdoor-work-card" }, hass).work.tasks;
  expect(def).toEqual([
...
test("carwash parked window needs both valid ends", () => {
  expect(wash({ parked_start: "08:00", parked_end: "16:30" }).wash.parked).toEqual([480, 990]);
